	...
	with open(schedule) as csvfile:
		...
		for row in csvreader:
			if (team in row[0]) or (team in row[1]):
				...
				if newhour > int(hour) and 'PM' in therest: #IF CHANGED FROM PM TO AM, REMOVE PM.  ADD AM
					therest = therest.split(' PM')[0] + ' AM'
				elif newhour > int(hour) and 'AM' in therest: #IF CHANGED FROM AM TO PM, REMOVE AM.  ADD PM
				...
				game[2] = monthtoint[month]+'/'+day+'/'+year
...
def clean(line):	
	...
	teamtoabbrev = {'HOME TEAM':'HOME', 'VISITING TEAM':'AWAY', 'ANAHEIM':'ANH', 'PHOENIX':'ARI', 'ARIZONA':'ARI', 'BOSTON':'BOS', 'BUFFALO':'BUF',  'CALGARY':'CGY', 'CAROLINA':'CAR', 'CHICAGO':'CHI', 'COLORADO':'COL', 'COLUMBUS':'CBJ', 'DALLAS':'DAL', 'DETROIT':'DET', 'EDMONTON':'EDM', 'FLORIDA':'FLA', 'LOS ANGELES':'LAK', 'MINNESOTA':'MIN', 'MONTREAL':'MON', 'NASHVILLE':'NSH', 'NEW JERSEY':'NJD', 'NY ISLANDERS':'NYI', 'NY RANGERS':'NYR', 'OTTAWA':'OTT', 'PHILADELPHIA':'PHI', 'PITTSBURGH':'PIT', 'SAN JOSE':'SJS', 'ST. LOUIS':'STL', 'TAMPA BAY':'TBL', 'TORONTO':'TOR', 'VANCOUVER':'VAN', 'WASHINGTON':'WSH', 'WINNIPEG':'WPG'}
	line[0] = teamtoabbrev[line[0]]
	line[1] = teamtoabbrev[line[1]]
	return line
...
leaguesched = []
empty = ['','','','']
csvfile = open(schedule) #open reader
csvreader = csv.reader(csvfile)
line = []
...
teamtimezone = {'ANH':'PT', 'ARI':'MT', 'BOS':'ET', 'BUF':'ET', 'CGY':'MT', 'CAR':'ET', 'CHI':'CT', 'COL':'MT', 'CBJ':'CT', 'DAL':'CT', 'DET':'CT', 'EDM':'MT', 'FLA':'ET', 'LAK':'PT', 'MIN':'CT', 'MON':'ET', 'NSH':'CT', 'NJD':'ET', 'NYI':'ET', 'NYR':'ET', 'OTT':'ET', 'PHI':'ET', 'PIT':'ET', 'SJS':'PT', 'STL':'CT', 'TBL':'ET', 'TOR':'ET', 'VAN':'PT', 'WSH':'ET', 'WPG':'CT'}
