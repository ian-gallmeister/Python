...
def addgames(date, team):
	returnstring = ''
	teamname = team.split('.')[0].upper() #xxx.csv -> ["xxx", "csv"] -> "xxx" -> "XXX"
...
for team in files:
	allteamschedule[0].append(team.split('.')[0].upper())
...
for rwo in allteamschedule:
	if rwo == [] or rwo[0] == 'DATE\\TEAM':
		...
		for team in files:
			#allteamschedule[0].append(team.split('.')[0].upper())
			rwo.append(addgames(rwo[0], team)) #MATCH date[0] & row[2], ADD OPPONENT IF MATCH == TRUE
	print rwo
